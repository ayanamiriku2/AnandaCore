...
# API calls go to localhost backend (internal port 3001)
...
# Backend runs on internal port 3001, Frontend runs on Railway's PORT
RUN echo '#!/bin/sh' > /app/start.sh && \
    echo 'echo "Starting AnandaCore Backend on port 3001..."' >> /app/start.sh && \
    echo 'APP_PORT=3001 APP_HOST=0.0.0.0 /app/anandacore &' >> /app/start.sh && \
